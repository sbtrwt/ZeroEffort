...
public class GetGameInformation : MonoBehaviour
{
    ...
    [SerializeField] private TextMeshProUGUI fpsObject, minFpsObject, maxFpsObject;
    ...
    private void Update()
    {
        if (showFps)
        {
            ...
            fpsObject.text = "FPS: " + (int)fps;

            UpdateColorValues(fpsObject,fps);

            if(showMinimumAndMaximumFps)
            {
                minFpsObject.text = "MIN FPS: " + (int)minFps;
                maxFpsObject.text = "MAX FPS: " + (int)maxFps;

                UpdateColorValues(minFpsObject,minFps);
                UpdateColorValues(maxFpsObject,maxFps);
            }
        }
    }
